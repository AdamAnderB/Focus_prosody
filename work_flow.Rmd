...
length(unique(flanker_cleaned$participant.private.id))
...
all_fixations %>%
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  scale_color_manual(values = c("#39FF14", "#FF10F0", "#FFFF33", "#9D00FF"))
...
unique(ge_stats_summary$time_cats_words_names)
View(ge_stats_summary)
...
fixation_stats_list$time_Dutch_word_gap
```
...
select_lmer_model <- function(df, dataset_name) {
  ...
  model_variants <- list(
    full = "lmer(fixation_proportion ~ AOI * condition + (1 + AOI * condition | participant.private.id), data = df, REML = FALSE)",
    no_slope = "lmer(fixation_proportion ~ AOI * condition + (1 | participant.private.id), data = df, REML = FALSE)",
    no_interaction = "lmer(fixation_proportion ~ AOI + condition + (1 | participant.private.id), data = df, REML = FALSE)",
    random_intercept_only = "lmer(fixation_proportion ~ AOI + condition + (1 | participant.private.id), data = df, REML = FALSE)",
...
best_models_flat <- best_models %>%
  unnest(cols = c(fixed_effects, random_effects))

# View results
View(best_models_flat)
```
