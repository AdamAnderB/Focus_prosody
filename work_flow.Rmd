...
battery_in_person<-battery%>%
 ...
 select(participant.private.id,experiment,tree.node.key,task.name,screen.name,screen.number,display,reaction.time,response,sound1,sound2,base1,base2,same)%>%
  ...
  mutate(responses=case_when(response == "'z'"~1,
         ...
         correct=if_else(responses==same,1,0),
         difference=abs(as.numeric(base1)-as.numeric(base2)))%>%
  filter(experiment=="data_exp_141883-v12"|
           experiment=="data_exp_161097-v1")%>%
...
battery_online<-battery%>%
  rename_with(tolower)%>%
  ungroup()%>%
  dplyr::filter(experiment!="data_exp_141883-v12"&
           experiment!="data_exp_161097-v1")
 select(participant.private.id,experiment,tree.node.key,task.name,screen.name,screen.number,display,reaction.time,response,sound1,sound2,base1,base2,same)
  filter(display=="block" &screen.name == "Screen 3")
  mutate(responses=case_when(response == "'z'"~1,
