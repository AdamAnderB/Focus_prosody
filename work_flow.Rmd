...
plots <- lapply(unique(ind_diff_raw_longer2$cog_measures), function(measure) {
  print(paste("Processing measure:", measure))  # Debugging step
...
# Function to extract summary details from a model
...
# Function to apply model selection to all datasets
...
# Flatten the model summary columns
library(purrr)

# Unnest fixed effects first (since it always has 4 rows)
...
best_models_flat <- best_model_flat %>%
  mutate(
    experiment_group = str_extract(dataset_name, "(?<=_)[^_]+"),  # Extract between first and second _
    time_cats_words = str_extract(dataset_name, "[^_]+$")         # Extract after last _
  )

# Convert time_cats_words into a factor with the specified levels
...
model_plot_dutch <- all_model_Data_viz %>%
  filter(experiment_group == "Dutch") %>%
  ggplot(aes(x = term, y = estimate, color = color, fill = whos)) +  # Use custom color column
...
model_plot_english <- all_model_Data_viz %>%
  ...
  # Titles and labels
  labs(title = "Model Output English",
       x = "Predictor Terms", 
       y = "Estimate", 
       color = "Effect Type") +
...
choose_best_gam <- function(model1, model2, plot_smooth = TRUE) {
  ...
  # Select the best model (lower AIC is better)
  ...
  # Print AIC/BIC values for comparison
  print(aic_values)
  print(bic_values)

  # Ensure the function returns the best model object
...
```{r}
library(broom)
library(broom.mixed)  # For handling GAMs properly

# Extract parametric (fixed effect) terms
...
all_best_fixed_effects <- bind_rows(
  ...
  mutate(
    Looks = str_extract(model, "^[^_]+"),  # Extracts the first part before "_"
  ...
  mutate(
    ...
    Phrase = factor(Phrase, levels = c("0", "1"), 
                    labels = c("The Rabbit is Verb1 the Object1", "Not Verb2 the Object2"))
  )%>%
  mutate(
    sig_label = case_when(
      p.value < 0.001 ~ "***",
...
gam_mod_out<-ggplot(all_best_fixed_effects, aes(x = term, y = estimate, color = color)) +
  geom_errorbar(aes(ymin = estimate - 1.96 *std.error, ymax = estimate +1.96 * std.error), width = 0.4,color="black") +
    geom_text(aes(label = sig_label,color=color), vjust = -0.5, size = 5, ) +  # Add significance stars
...
scale_column <- function(x) {
  ...
  max_abs_x <- max(abs(x - mean_x), na.rm = TRUE)  # Use max absolute deviation for scaling
  if (max_abs_x == 0) {
    return(rep(0, length(x)))  # Avoid division by zero if all values are identical
  }
  return((x - mean_x) / max_abs_x)
}

# Apply scaling to individual difference measures
...
# Function to automate LASSO-based GAM selection
run_lasso_gam <- function(data, response, time_var, random_effect, alpha = 1) {
  ...
  for (var in fixed_effects) {
    if (is.factor(data[[var]])) {
      data[[var]] <- as.numeric(as.factor(data[[var]]))  # Convert to numeric levels
    }
  }
  
  # Create interaction terms manually for LASSO
  interaction_terms <- c(
    "experiment_group * condition", 
    "dprime_battery_up_dur * duration", 
    "dprime_battery_up_formants * stress_score", 
  ...
  # Extract nonzero coefficients and clean variable names
  ...
  selected_vars <- gsub("[0-9]+", "", selected_vars)  # Remove factor level suffixes
  
  # Step 3: Construct GAM formula
...
id_gam <- bind_rows(comp_0_best_fixed_effects,comp_1_best_fixed_effects,
  ...
  mutate(
    Looks = str_extract(model, "^[^_]+"),  # Extracts the first part before "_"
    Phrase = str_extract(model, "(?<=_)[^_]+")  # Extracts after "_"
  )%>%
  mutate(
    color = case_when(
