...
#functions for keeping up with cb and participant removal
```{r}
...
track_cronbach_alpha <- function(existing_df = NULL, ...) {
  ...
  new_data <- data.frame(
    Task = dataset_names,
    Cronbachs_Alpha = unlist(datasets),
...
battery_tidy<-battery%>%
  rename_with(tolower)%>%
  ungroup()%>%
 select(participant.private.id,experiment,tree.node.key,task.name,screen.name,screen.number,display,reaction.time,response,sound1,sound2,base1,base2,same)%>%
...
battery_cleaned<-battery_tidy%>%
  ...
  mutate(correct = ifelse(is.na(correct), 0, correct),
         log_rt=sqrt(as.numeric(reaction.time)))%>%
...
battery_agg_agg<-battery_cleaned%>%
  group_by(participant.private.id,experiment,task.name)%>%
  summarize(mean=mean(correct))
  
battery_agg_agg%>%ggplot(aes(x=participant.private.id,y=mean))+
  geom_point()+
  facet_wrap(task.name~experiment)
```
...
battery_agg2<-battery_cleaned%>%
  group_by(participant.private.id, task.name,experiment) %>%
...
#  family = wiener(),
#  cores = 4, iter = 10000, warmup = 1000, chains = 4
#)
...
#ddm_model_10<-ddm_model
# save fit  model:
save_ddm_model(ddm_model_10)

# Load the most recent model into ddm_model
ddm_model <- load_ddm_model()
#ddm_model <- readRDS("../ddm_output_1.rds")
...
part_keep_rem_agg_lang_stand<-part_keep_rem_agg%>%
  filter(mean_presence==1)


colnames(part_keep_rem_agg_lang_stand)
agg_removal<-part_keep_rem_agg%>%
  pivot_longer(-participant.private.id,
               names_to = "task",
               values_to = "values")%>%
  filter(values==0)%>%
  group_by(task)%>%
...
write.csv(part_keep_rem_agg_lang_stand, "part_keep.csv", row.names = FALSE)
write.csv(agg_removal, "agg_part_keep.csv", row.names = FALSE)
write.csv(part_keep_rem_agg, "part_keep_rem_agg.csv", row.names = FALSE)
```
...
write.csv(cronbachs_alpha, "cronbachs_alpha.csv", row.names = FALSE)
```


individual differences cleanup together
```{r}
ind_diff<-lang_back_simple%>%
  ...
  filter(participant.private.id %in% part_keep_rem_agg_lang_stand$participant.private.id)
...
indiviudal differences visualizations
```{r,warning=FALSE}
...
cog_measure_labels <- c(
  "dprime_battery_up_dur" = "Duration d'",
  "dprime_battery_up_formants" = "Formants d'",
  "dprime_battery_up_ptich" = "Pitch d'",
  "dprime_battery_up_risetime" = "Risetime d'",
  "mean_working_memory" = "Work-mem",
...
ggplot(ind_diff_raw_longer2 %>% filter(cog_measures == unique(ind_diff_raw_longer2$cog_measures)[1]), 
       aes(x = 0, y = scores)) +
  geom_half_violin(aes(fill = exp, group = interaction(exp, cog_measures)), 
                   color = "white", alpha = .8, position = "identity", side = "right") +
...
plots <- lapply(unique(ind_diff_raw_longer2$cog_measures), function(measure) {
  print(paste("Processing measure:", measure))
  ...
  ggplot(ind_diff_raw_longer2 %>% filter(cog_measures == measure), 
    ...
    scale_fill_manual(values = c("dutch" = "black", "english" = "grey80"),  
                    labels = c("dutch" = "L1 Dutch - L2 Dutch", "english" = "L1 English")) +  
...
p1<-ggplot(ind_diff_acoust_longer3, aes(
  ...
  geom_hline(yintercept = 26, color = "white", linewidth = 12)+
...
```{r}
part_rawers<-lang_back_simple%>%
  group_by(participant.private.id,exp)%>%
  unique()%>%
  ungroup()%>%
  group_by(exp)%>%
  count()
...
part_keepers<-ind_diff_acoust_longer3%>%
  ungroup()%>%
  select(participant.private.id,exp)%>%
  group_by(participant.private.id,exp)%>%
  unique()%>%
  ungroup()%>%
  group_by(exp)%>%
...
background_plot <- (plot1 | plot3) + plot_layout(guides = "collect") &
  theme(legend.position = "none")
background_plot
```
...
ggsave("viz/combined_plot_circle.pdf", combined_plot, width = 12, height = 14, dpi = 300)
ggsave("viz/combined_plot_circle.png", combined_plot, width = 12, height = 14, dpi = 300)
```
...
beahvioral data and ge et al.(2021)
```{r}
...
nrow(ge_data)/nrow(ge_et_data)
ge_data%>%
  ggplot(aes(x=x_pred_normalised,y=y_pred_normalised,color=quadrant))+
  geom_point()+
  theme_minimal()
```
...
frame_rate<-ge_data_mapped_clean%>%
  group_by(participant.private.id,object)%>%
  summarize(max_time=max(time),
            min_time=min(time),
            length=(max_time-min_time)/1000,
            count=n())%>%
  mutate(fr=count/length)


fr_viz<-frame_rate %>%
  filter(is.finite(fr), fr > 0)%>%
  ggplot(aes(x = reorder(as.numeric(participant.private.id), fr, FUN = mean), y = fr)) +
  geom_hline(yintercept = mean(frame_rate$fr[is.finite(frame_rate$fr) & frame_rate$fr > 0]),
             color = "blue", linetype = "solid", size = 1) +
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = 0, ymax = 5),
            fill = "grey90", alpha = 0.5, inherit.aes = FALSE) +
  geom_violin(fill = "grey80", color = "grey50") +
  ...
  labs(
    x = "Participant ID",
    y = "Frame rate (Hz)",
    title = "Distribution of Frame Rates by Participant") +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 90, hjust = 1))

fr_viz
ggsave("viz/fr.png", fr_viz, width = 10, height = 5, dpi = 300)
...
```{r}
#list.files("../data/")
#write.csv(fixation_proportions , file = file.path("../data", "fixation_proportions.csv"), row.names = FALSE)
ge_estimates<-read.csv("../data/fixations_estimates_ge_et_al.csv")
ge_estimates$whos<-"Ge et al. (2021)"

fixation_proportions$whos<-"Ours"

all_fixations<-rbind(fixation_proportions,ge_estimates)

all_fixations$time_cats_words <- factor(all_fixations$time_cats_words, 
  levels = c("         ", "The rabbit is", "only", "verb1", "the1", 
             "object1", "  ", "not", "verb2", "the2", "object2", " "),
  ordered = TRUE
)
```
...
all_fixations <- all_fixations %>%
  mutate(category = factor(category, levels = c("target", "obj", "verb", "dist")))%>%
  na.omit()


dutch_fix<-all_fixations %>%
  filter(experiment_group == "Dutch") %>%
  mutate(experiment_group = ifelse(experiment_group=="Dutch",
                                   "L1 Dutch - L2 English","L1 English"))%>%
  ggplot(aes(x = time_cats_words, y = fixation_proportion, group = interaction(condition, experiment_group))) +
  geom_line(color = "black", linewidth=1, alpha=.5, show.legend = FALSE) +
  geom_point(aes(shape = experiment_group), color = "white", size=2) +
  geom_point(aes(color = condition, shape = experiment_group)) +
  geom_errorbar(aes(ymin = fixation_proportion - se, 
                    ymax = fixation_proportion + se, 
                    color = condition), 
                width = 0.2, alpha = 0.5) +
  facet_grid(whos ~ category, scales = "free_x", space = "free",
             labeller = labeller(category = c("dist" = "Distractor",
                                              "target" = "Target",
                                              "obj" = "Object Stressed Competitor",
                                              "verb" = "Verb Stressed Competitor"))) +
  theme_minimal() +
  theme(
  ...
  labs(y = "Fixation Proportion",
       x = "") +  # Corrected way to rename the y-axis
  scale_color_manual(values = c("#0D47A1", "#FF6F00"),
                     labels = c("Object Stressed", "Verb Stressed"))+
  guides(color = guide_legend(title = "Condition:"),
       linetype = guide_legend(title = "Language:"))


english_fix<-all_fixations %>%
  filter(experiment_group != "Dutch") %>%
  mutate(experiment_group = ifelse(experiment_group=="Dutch",
                                   "L1 Dutch - L2 English","L1 English"))%>%
  ggplot(aes(x = time_cats_words, y = fixation_proportion, group = interaction(condition, experiment_group))) +
  geom_line(color = "grey80", linewidth=1, alpha=.5, show.legend = FALSE) +
  geom_point(aes(shape = experiment_group), color = "white", size=2) +
  geom_point(aes(color = condition, shape = experiment_group)) +
  geom_errorbar(aes(ymin = fixation_proportion - se, 
                    ymax = fixation_proportion + se, 
...
english_fix
dutch_fix
ggsave("viz/dutch_fix.png", dutch_fix, width = 10, height = 6, dpi = 300)
ggsave("viz/english_fix.png", english_fix, width = 10, height = 6, dpi = 300)
```

```{r}

dutch_fix2<-all_fixations %>%
  filter(experiment_group == "Dutch") %>%
  mutate(experiment_group = ifelse(experiment_group=="Dutch",
                                   "L1 Dutch - L2 English","L1 English"))%>%
  ggplot(aes(x = time_cats_words, y = fixation_proportion, color = category, group = interaction(category, whos))) +
  ...
  geom_errorbar(aes(ymin = fixation_proportion - se, 
                    ymax = fixation_proportion + se), 
                width = 0.2, alpha = 0.5) +
  facet_grid(whos ~ condition, scales = "free_y",
             labeller = labeller(condition = c("verb_stressed" = "Verb Stressed",
                                              "object_stressed" = "Object Stressed"))) +
  theme_minimal() +
  theme(legend.position = "top",
        axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "L1 Dutch - L2 English",
       x = "",
       y = "Fixation proportions") +
  scale_color_manual(values = c("#39FF14", "#9D00FF", "#FFFF33","#FF10F0"),
                     labels = c("dist" = "Distractor",
                                "target" = "Target",
                                "obj" = "Object Stressed Competitor",
                                "verb" = "Verb Stressed Competitor"))


english_fix2<-all_fixations %>%
  filter(experiment_group != "Dutch") %>%
  mutate(experiment_group = ifelse(experiment_group=="Dutch",
                                   "L1 Dutch - L2 English","L1 English"))%>%
  ggplot(aes(x = time_cats_words, y = fixation_proportion, color = category, group = interaction(category, whos))) +
...
ggsave("viz/dutch_fix2.png", dutch_fix2, width = 10, height = 10, dpi = 300)
ggsave("viz/english_fix2.png", english_fix2, width = 10, height = 10, dpi = 300)
```

```{r}

fixation_stats<- ge_data_mapped_clean %>%
  #mutate(experiment_place=if_else(experiment=="data_exp_142778-v2","prolific","in-person"))%>%
  mutate(experiment_group=if_else(experiment=="data_exp_213779-v3","Dutch","English"))%>%
  ungroup()%>%
  select(participant.private.id,condition,time_cats,time_cats_words,
         experiment_group,target_looks,obj_comp_looks,
         verb_comp_looks,dist_looks)%>%
  pivot_longer(
    cols = c(ends_with("_looks")),
    names_to = c("category")
  )%>%
  mutate(time_cats_words = case_when(
      ...
      time_cats == 12 ~ "offset",
      TRUE ~ NA_character_))%>%
    filter(time_cats_words=="verb1"|time_cats_words=="the1"|
             time_cats_words=="object1"|time_cats_words=="gap"|
             time_cats_words=="not"|time_cats_words=="verb2"|
             time_cats_words=="the2"|time_cats_words=="object2"|
             time_cats_words=="offset")%>%
  group_by(participant.private.id,condition,time_cats_words,time_cats,experiment_group,category)%>%
  summarize(fixation_proportion=mean(value))%>%
  filter(category=="obj_comp_looks"|category=="verb_comp_looks")%>%
  mutate(AOI=as.factor(category))

split_by_time_steps <- function(df) {
  grouped_data <- df %>% group_by(experiment_group, time_cats_words) %>% group_split()
  
  # Extract unique names for each subset
  names(grouped_data) <- df %>%
    group_by(experiment_group, time_cats_words) %>%
    summarise() %>%
    mutate(name = paste0("time_", experiment_group, "_word_", time_cats_words)) %>%
    pull(name)
...
fixation_stats$condition <- factor(fixation_stats$condition, levels = c("object_stressed", setdiff(unique(fixation_stats$condition), "object_stressed")))

# Apply the function to your dataset
fixation_stats_list <- split_by_time_steps(fixation_stats)
levels(fixation_stats_list$time_Dutch_word_gap$condition)
unique(fixation_stats_list$time_Dutch_word_gap$condition)
```
...
extract_model_summary <- function(model) {
  if (is.null(model)) return(list(fixed_effects = NULL, random_effects = NULL, model_info = NULL))

  model_type <- ifelse(inherits(model, "lmerMod"), "lmer", "lm")

  fixed_effects <- tryCatch(tidy(model, effects = "fixed"), error = function(e) NULL)

  # Extract random effects (only for lmer models)
  random_effects <- if (model_type == "lmer") {
    tryCatch(tidy(model, effects = "ran_pars"), error = function(e) NULL)
  } else {
    NULL
  }

  model_info <- tryCatch(
    glance(model) %>% select(AIC, BIC, logLik) %>% mutate(model_type = model_type),
...
# best model
select_lmer_model <- function(df, dataset_name) {
  if (n_distinct(df$participant.private.id) < 2) {
    return(tibble(
      ...
      fixed_effects = list(NULL),
      random_effects = list(NULL)
    ))
  }

  model_variants <- list(
    full = "lmer(fixation_proportion ~ AOI * condition + (1 + AOI * condition | participant.private.id), 
                  data = df, REML = FALSE, 
                  control = lmerControl(optimizer = 'bobyqa', optCtrl = list(maxfun = 100000)))",
    no_slope = "lmer(fixation_proportion ~ AOI * condition + (1 | participant.private.id), 
                       data = df, REML = FALSE, 
                       control = lmerControl(optimizer = 'bobyqa', optCtrl = list(maxfun = 100000)))",
    no_interaction = "lmer(fixation_proportion ~ AOI + condition + (1 | participant.private.id), 
                             data = df, REML = FALSE, 
                             control = lmerControl(optimizer = 'bobyqa', optCtrl = list(maxfun = 100000)))",
    random_intercept_only = "lmer(fixation_proportion ~ AOI + condition + (1 | participant.private.id), 
                                    data = df, REML = FALSE, 
                                    control = lmerControl(optimizer = 'bobyqa', optCtrl = list(maxfun = 100000)))",
    lm_fallback = "lm(fixation_proportion ~ AOI * condition, data = df)"
  )

  model_results <- list()

  for (model_name in names(model_variants)) {
    model <- tryCatch(eval(parse(text = model_variants[[model_name]])), error = function(e) NULL)

    if (!is.null(model)) {
      converged <- ifelse(inherits(model, "lmerMod"), length(model@optinfo$conv$lme4$messages) == 0 && !isSingular(model), TRUE)
      ...
      model_results[[model_name]] <- tibble(
        ...
        message = paste("Model attempted:", model_name),
        converged = converged,
        model_type = ifelse(inherits(model, "lmerMod"), "lmer", "lm"),
        AIC = model_summary$model_info$AIC,
        BIC = model_summary$model_info$BIC,
        logLik = model_summary$model_info$logLik,
        fixed_effects = list(model_summary$fixed_effects),
        random_effects = list(model_summary$random_effects)
      )

      if (converged) {
        model_results[[model_name]]$message <- paste("Selected model:", model_name)
        break
      }
    }
  }
...
run_lmer_selection <- function(fixation_stats_list) {
  results <- pblapply(names(fixation_stats_list), function(name) {
    select_lmer_model(fixation_stats_list[[name]], dataset_name = name)
  })
...
# model selection
fixation_model_results <- run_lmer_selection(fixation_stats_list)

best_models <- fixation_model_results %>%
...
best_model_flat <- best_models %>%
  select(dataset_name, formula, model_type, fixed_effects) %>%
  unnest(cols = fixed_effects, keep_empty = TRUE)

best_models_flat <- best_model_flat %>%
  mutate(
    experiment_group = str_extract(dataset_name, "(?<=_)[^_]+"),
    time_cats_words = str_extract(dataset_name, "[^_]+$") 
  )

best_models_flat$time_cats_words <- factor(best_models_flat$time_cats_words, 
  levels = c("The rabbit is", "only", "verb1", "the1", 
             "object1", "gap", "not", "verb2", "the2", "object2", "offset")
)

best_models_flat_cleaned<-best_models_flat%>%
  select("time_cats_words","experiment_group","formula","model_type","term", "estimate","std.error","statistic","p.value")
best_models_flat_cleaned$whos<-"Ours"

View(best_models_flat_cleaned)

ge_stats_summary<-read.csv("~/scripts/music_project_23/FoCr/data/ge_stats_summary3.csv")

ge_stats_summary_cleaned <- ge_stats_summary %>%
  rename(
    time_cats_words = time_cats_words_names,
    term = Predictor,
    estimate = Estimate,
    std.error = SE,
    statistic = t.value,
    p.value = p.value,
    experiement_group = Language)%>%
  mutate(time_cats_words=ifelse(time_cats_words=="offset500","offset",time_cats_words),
         experiment_group=experiement_group)

ge_stats_summary_cleaned$whos<-"Ge et al. (2021)"


all_cols <- union(colnames(best_models_flat_cleaned), colnames(ge_stats_summary_cleaned))

best_models_flat_cleaned[setdiff(all_cols, colnames(best_models_flat_cleaned))] <- NA
ge_stats_summary_cleaned[setdiff(all_cols, colnames(ge_stats_summary_cleaned))] <- NA

best_models_flat_cleaned <- best_models_flat_cleaned[, all_cols]
ge_stats_summary_cleaned <- ge_stats_summary_cleaned[, all_cols]
```

```{r}
library(stringr)

all_model_Data_viz <- rbind(best_models_flat_cleaned, ge_stats_summary_cleaned) %>%
  mutate(
    sig_level = case_when(
      p.value < 0.001 ~ "***",
      p.value < 0.01 ~ "**",
    ...
    time_cats_words = factor(time_cats_words, 
      levels = c("The rabbit is", "only", "verb1", "the1", 
                 "object1", "gap", "not", "verb2", "the2", "object2", "offset"))
  )%>%
  mutate(term = str_replace_all(as.character(term), "verb_comp_looks|verb_stressed", ""))%>%
  mutate(term = str_replace_all(term, "condition", "Condition"))%>%
  mutate(color = case_when(
    term == "Condition" & estimate < 0 ~ "#0D47A1",  # Condition negative: Blue
    term == "Condition" & estimate > 0 ~ "#FF6F00",  # Condition positive: Orange
    term == "AOI" & estimate < 0 ~ "#9D00FF",        # AOI negative: Purple
    term == "AOI" & estimate > 0 ~ "#FFFF33",        # AOI positive: Yellow
    term =="(Intercept)"~"grey",
    TRUE ~ "black"                                    # All others: Black
  ))
write.csv(all_model_Data_viz, file = "../data/all_model_Data_viz.csv", row.names = FALSE)
```

```{r}

model_plot_dutch <- all_model_Data_viz %>%
  filter(experiment_group == "Dutch") %>%
  ggplot(aes(x = term, y = estimate, color = color, fill = whos)) +
  geom_point(size = 3, alpha = 0.7) +
  geom_errorbar(aes(ymin = estimate - std.error, ymax = estimate + std.error), width = 0.2, alpha = 0.7) +
  geom_text(aes(label = sig_level), vjust = -1, size = 4, color = "black") +
  labs(title = "Model Output L1 English - L2 Dutch",
       x = "Predictor Terms", 
       y = "Estimate", 
       color = "Effect Type") +
  theme_minimal() +
  ...
  facet_grid(whos ~ time_cats_words, scales = "free_x", space = "free_x") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", linewidth = 0.6) +
  scale_color_identity()

model_plot_english <- all_model_Data_viz %>%
  filter(experiment_group == "English") %>%
  ggplot(aes(x = term, y = estimate, color = color, fill = whos)) + 
  geom_point(size = 3, alpha = 0.7) +
  geom_errorbar(aes(ymin = estimate - 1.96 *std.error, ymax = estimate + 1.96 *std.error), width = 0.2, alpha = 0.7) +
  geom_text(aes(label = sig_level), vjust = -1, size = 4, color = "black") +
  labs(title = "Model Output L1 English",
       x = "Predictor Terms", 
       y = "Estimate", 
       color = "Effect Type") +
  theme_minimal() +
...
ggsave("viz/model_plot_english.png", model_plot_english, width = 10, height = 5, dpi = 300)
ggsave("viz/model_plot_dutch.png", model_plot_dutch, width = 10, height = 5, dpi = 300)
```

```{r}

gam_stats<- ge_data_mapped_clean %>%
  #mutate(experiment_place=if_else(experiment=="data_exp_142778-v2","prolific","in-person"))%>%
  mutate(experiment_group=if_else(experiment=="data_exp_213779-v3","Dutch","English"))%>%
  ungroup()%>%
  select(participant.private.id,condition,time_cats,time_cats_words,
         experiment_group,target_looks,obj_comp_looks,
         verb_comp_looks,dist_looks,audio)%>%
  pivot_longer(
    cols = c(ends_with("_looks")),
    names_to = c("category")
  )%>%
  mutate(time_cats_words = case_when(
      ...
      time_cats == 12 ~ "offset",
      TRUE ~ NA_character_))%>%
    filter(time_cats_words=="The rabbit is"|time_cats_words=="only"|
             time_cats_words=="verb1"|time_cats_words=="the1"|
             time_cats_words=="object1"|time_cats_words=="gap"|
             time_cats_words=="not"|time_cats_words=="verb2"|
             time_cats_words=="the2"|time_cats_words=="object2"|
             time_cats_words=="offset")%>%
  ...
  mutate(AOI=as.factor(category))%>%
  mutate(
    experiment_group = factor(experiment_group, levels = c("English", "Dutch")),
    condition = factor(condition, levels = c("object_stressed", "verb_stressed")),
    participant.private.id=as.factor(participant.private.id),
    )%>%
  mutate(value = as.numeric(as.character(value)))%>%
  mutate(audio=as.factor(audio))%>%
  mutate(time_cats_words = factor(time_cats_words, 
      levels = c("The rabbit is", "only", "verb1", "the1", 
                 "object1", "gap", "not", "verb2", "the2", "object2","offset")))%>%
  mutate(sentence_phase = as.factor(case_when(time_cats < 6~0,
                                    time_cats>6~1)))%>%
  filter(time_cats!=6)
  

target_gam <- gam_stats %>% filter(AOI == "target_looks")%>%na.omit()
target_gam_0<-target_gam%>%filter(sentence_phase==0)
target_gam_1<-target_gam%>%filter(sentence_phase==1)

competitor_gam <- gam_stats %>%
  filter((condition == "verb_stressed" & AOI == "verb_comp_looks") |
         (condition == "object_stressed" & AOI == "obj_comp_looks"))
competitor_gam_0<-competitor_gam %>%filter(sentence_phase==0)
competitor_gam_1<-competitor_gam %>%filter(sentence_phase==1)
```

```{r}
choose_best_gam <- function(model1, model2, plot_smooth = TRUE) {
  aic_values <- AIC(model1, model2)
  bic_values <- BIC(model1, model2)
  if (aic_values$AIC[1] < aic_values$AIC[2]) {
    best_model <- model1
  ...
  print(aic_values)
  print(bic_values)
  message("Best model selected: ", best_model_name)
  return(best_model)
}
...
```{r}
library(mgcv)
#target_gam_model0 <- bam(value ~ s(time_cats,by = experiment_group) + experiment_group * condition +
#                         s(participant.private.id, bs = "re") + 
#                         s(audio, participant.private.id, bs = "re"),
...
# Summary of model
#summary(target_gam_model)

target_gam_model0_1 <- bam(value ~ time_cats * experiment_group * condition +
                         s(participant.private.id, bs = "re"),
...
# Summary of model
summary(target_gam_model0_1)

target_gam_model0_2 <- bam(value ~ time_cats + experiment_group + condition +
                         s(participant.private.id, bs = "re"),
                         family = binomial(link = "logit"),
                         data = target_gam_0,
                         method = "fREML")
...
# Summary of model
summary(target_gam_model0_2)


target_gam_model1_1 <- bam(value ~ time_cats * experiment_group * condition +
                         s(participant.private.id, bs = "re"),
                         family = binomial(link = "logit"),
                         data = target_gam_1,
                         method = "fREML")
...
# Summary of model
summary(target_gam_model1_1)

target_gam_model1_2 <- bam(value ~ time_cats + experiment_group + condition +
                         s(participant.private.id, bs = "re"),
                         family = binomial(link = "logit"),
                         data = target_gam_1,
                         method = "fREML")
...
# Summary of model
summary(target_gam_model1_2)


targ_0_best<-choose_best_gam(target_gam_model0_1,target_gam_model0_2)
targ_1_best<-choose_best_gam(target_gam_model1_1,target_gam_model1_2)
```

```{r}
#competitor_gam_model0 <- bam(value ~ s(time_cats,by = experiment_group) + time_cats*experiment_group * condition +
#                         s(participant.private.id, bs = "re") + 
#                         s(audio, participant.private.id, bs = "re"),
#                         family = binomial(link = "logit"),  # Logistic regression
#                         data = competitor_gam,
...
# Summary of model
#summary(competitor_gam_model)


competitor_gam_model0_1 <- bam(value ~ time_cats * experiment_group * condition +
                         s(participant.private.id, bs = "re"),
                         family = binomial(link = "logit"),
                         data = competitor_gam_0,
                         method = "fREML")



# Summary of model
summary(competitor_gam_model0_1)

competitor_gam_model0_2 <- bam(value ~ time_cats + experiment_group + condition +
                         s(participant.private.id, bs = "re"),
                         family = binomial(link = "logit"),
                         data = competitor_gam_0,
                         method = "fREML")
...
# Summary of model
summary(competitor_gam_model0_2)


competitor_gam_model1_1 <- bam(value ~ time_cats * experiment_group * condition +
                         s(participant.private.id, bs = "re"),
                         family = binomial(link = "logit"),
                         data = competitor_gam_1,
                         method = "fREML")



# Summary of model
summary(competitor_gam_model1_1)

competitor_gam_model1_2 <- bam(value ~ time_cats + experiment_group + condition +
                         s(participant.private.id, bs = "re"),
                         family = binomial(link = "logit"),
                         data = competitor_gam_1,
                         method = "fREML")
...
# Summary of model
summary(competitor_gam_model1_2)

levels(target_gam_0$experiment_group)
comp_0_best<-choose_best_gam(competitor_gam_model0_1,competitor_gam_model0_2)
comp_1_best<-choose_best_gam(competitor_gam_model1_1,competitor_gam_model1_2)
```

```{r}
library(broom)
library(broom.mixed)  
targ_0_best_fixed_effects <- tidy(targ_0_best, parametric = TRUE) %>%
  mutate(model = "targ_0_best")

targ_1_best_fixed_effects <- tidy(targ_1_best, parametric = TRUE) %>%
  mutate(model = "targ_1_best")

comp_0_best_fixed_effects <- tidy(comp_0_best, parametric = TRUE) %>%
  mutate(model = "comp_0_best")

comp_1_best_fixed_effects <- tidy(comp_1_best, parametric = TRUE) %>%
  mutate(model = "comp_1_best")

all_best_fixed_effects <- bind_rows(
  targ_0_best_fixed_effects,
  targ_1_best_fixed_effects,
  comp_0_best_fixed_effects,
  comp_1_best_fixed_effects)%>%
  
  mutate(
    Looks = str_extract(model, "^[^_]+"), 
    Phrase = str_extract(model, "(?<=_)[^_]+"))%>%
  mutate(
    color = case_when(
      term == "experiment_groupDutch" & estimate > 0 ~ "black",
      term == "experiment_groupDutch" & estimate <= 0 ~ "grey80",  
      term == "conditionverb_stressed" & estimate > 0 ~ "#FF6F00", 
      term == "conditionverb_stressed" & estimate <= 0 ~ "#0D47A1", 
      term == "time_cats:experiment_groupDutch" & estimate > 0 ~ "black", 
      term == "time_cats:experiment_groupDutch" & estimate <= 0 ~ "grey80", 
      term == "time_cats:conditionverb_stressed" & estimate > 0 ~ "#FF6F00",
      term == "time_cats:conditionverb_stressed" & estimate <= 0 ~ "#0D47A1",
      TRUE ~ "white"
    )
  )%>%
  mutate(
    term = case_when(
      ...
      term == "time_cats" ~ "Time",
      term == "experiment_groupDutch" ~ "L1",
      term == "conditionverb_stressed" ~ "Stress",
      term == "time_cats:experiment_groupDutch" ~ "Time:L1",
      term == "time_cats:conditionverb_stressed" ~ "Time:Stress",
      term == "experiment_groupDutch:conditionverb_stressed" ~ "L1:Stress",
      term == "time_cats:experiment_groupDutch:conditionverb_stressed" ~ "Time:L1:Stress",
      TRUE ~ term))%>%
  mutate(
    Looks = factor(Looks, levels = c("targ", "comp"), 
                   labels = c("Target looks", "Competitor looks")),
    Phrase = factor(Phrase, levels = c("0", "1"), 
                    labels = c("The Rabbit is only Verb1 the Object1", "Not Verb2 the Object2"))
  )%>%
  mutate(
    sig_label = case_when(
      p.value < 0.001 ~ "***",
...
gam_mod_out<-ggplot(all_best_fixed_effects, aes(x = term, y = estimate, color = color)) +
  geom_errorbar(aes(ymin = estimate - 1.96 *std.error, ymax = estimate +1.96 * std.error), width = 0.4,color="black") +
      geom_text(aes(label = sig_label),color="black", vjust = -0.8, size = 5, ) + 
  ...
  facet_grid(Looks ~ Phrase)

gam_mod_out
ggsave("viz/gam_mod_out.png", gam_mod_out, width = 12, height = 6, dpi = 300)

View(all_best_fixed_effects)
write.csv(all_best_fixed_effects, file = "../data/gam_models.csv", row.names = FALSE)
```
...
scaled_ind_diff <- ind_diff_simp %>%
  select("participant.private.id", 
         "dprime_battery_up_dur", "dprime_battery_up_formants",
         "dprime_battery_up_ptich", "dprime_battery_up_risetime", 
         "mean_working_memory", "lextale_score", "melody_score", 
         "beat_score", "drift_rate") %>%
  mutate(across(c("dprime_battery_up_dur", "dprime_battery_up_formants",
                  "dprime_battery_up_ptich", "dprime_battery_up_risetime", 
                  "mean_working_memory", "lextale_score", "melody_score", 
                  "beat_score", "drift_rate"), scale_column))


acc<-read.csv("../data/scaled_acoustics.csv")
acc_cl<-acc%>%
  mutate(
    audio = as.factor(str_extract(word, "^[^_]+") %>% paste0(".wav")), 
    time_cats = as.numeric(str_extract(word, "[^_]+$")))
  
id_stats<-gam_stats%>%
  left_join(scaled_ind_diff)%>%
  left_join(acc_cl)%>%
  select("participant.private.id", 
  "time_cats", "time_cats_words", 
  "experiment_group", "audio", 
  "category", "value", "condition", 
  "AOI", "sentence_phase", "dprime_battery_up_dur", 
  "dprime_battery_up_formants", "dprime_battery_up_ptich", 
  "dprime_battery_up_risetime", "mean_working_memory", 
  "lextale_score", "melody_score", 
  "beat_score", "drift_rate", "duration", 
  "pitch_range", "stress_score", 
  "amplitude_raw")%>%
  mutate(participant.private.id=as.factor(participant.private.id))%>%na.omit()

target_id <- id_stats %>% filter(AOI == "target_looks")%>%na.omit()
target_id_0<-target_id%>%filter(sentence_phase==0)
target_id_1<-target_id%>%filter(sentence_phase==1)

competitor_id<- id_stats %>%
  filter((condition == "verb_stressed" & AOI == "verb_comp_looks") |
         (condition == "object_stressed" & AOI == "obj_comp_looks"))
competitor_id_0<-competitor_id %>%filter(sentence_phase==0)
competitor_id_1<-competitor_id %>%filter(sentence_phase==1)
...
write.csv(id_stats, file = "data/full_data.csv", row.names = FALSE)
```


#gam example for id differences
```{r}
library(mgcv)
library(glmnet)

run_lasso_gam <- function(data, response, time_var, random_effect, alpha = 1) {

  fixed_effects <- c(
    "experiment_group", "condition", 
    "dprime_battery_up_dur", "duration", 
    "dprime_battery_up_formants", "stress_score", 
    "dprime_battery_up_ptich", "pitch_range", 
    "dprime_battery_up_risetime", "amplitude_raw", 
    "pitch_range", "melody_score", 
    "duration", "beat_score", 
    "mean_working_memory", "drift_rate", "lextale_score"
  )
  ...
  # LASSO matrix
  lasso_data <- model.matrix(as.formula(paste(response, "~", paste(fixed_effects, collapse = " + "), " + ", 
                                              paste(interaction_terms, collapse = " + "))), 
                             data = data)
  
  # Convert response to numeric (for binomial LASSO)
  y <- as.numeric(data[[response]])

  lasso_model <- cv.glmnet(lasso_data, y, family = "binomial", alpha = alpha)
  ...
  selected_vars <- gsub("[0-9]+", "", selected_vars)  
  gam_formula <- paste(response, "~ s(", time_var, ", k = 5) + ", 
                        paste(selected_vars, collapse = " + "), " + ", 
                        paste0("s(", random_effect, ", bs = 're')"))

  gam_model <- bam(as.formula(gam_formula), family = binomial(link = "logit"),
                   data = data, method = "fREML")
  
  return(gam_model)
}

out0<-run_lasso_gam(target_id_0, response = "value", time_var = "time_cats", random_effect = "participant.private.id")

out1<-run_lasso_gam(target_id_1, response = "value", time_var = "time_cats", random_effect = "participant.private.id")
 
comp_out0<-run_lasso_gam(competitor_id_0, response = "value", time_var = "time_cats", random_effect = "participant.private.id")

comp_out1<-run_lasso_gam(competitor_id_1, response = "value", time_var = "time_cats", random_effect = "participant.private.id")
...
library(broom.mixed)
targ_0_best_fixed_effects <- tidy(out0, parametric = TRUE) %>%
  mutate(model = "targ_0_best")

targ_1_best_fixed_effects <- tidy(out1, parametric = TRUE) %>%
  mutate(model = "targ_1_best")

comp_0_best_fixed_effects <- tidy(comp_out0, parametric = TRUE) %>%
  mutate(model = "comp_0_best")

comp_1_best_fixed_effects <- tidy(comp_out1, parametric = TRUE) %>%
  mutate(model = "comp_1_best")


unique(id_gam$term)
id_gam <- bind_rows(comp_0_best_fixed_effects,comp_1_best_fixed_effects,
                    targ_0_best_fixed_effects,targ_1_best_fixed_effects)%>%
  mutate(
    Looks = str_extract(model, "^[^_]+"), 
    Phrase = str_extract(model, "(?<=_)[^_]+")
  )%>%
  mutate(
    color = case_when(
      p.value >= 0.05 ~ "white",
      str_detect(term, "dprime_battery_up_dur") ~ "#E63946",
      str_detect(term, "dprime_battery_up_formants") ~ "#2A9D8F",
      str_detect(term, "dprime_battery_up_ptich") ~ "#F4A261",
      str_detect(term, "dprime_battery_up_risetime") ~ "#264653",
      str_detect(term, "lextale_score") ~ "#bc5090",
      str_detect(term, "mean_working_memory") ~ "grey",
      str_detect(term, "drift_rate") ~ "#ffa600",
      str_detect(term, "melody_score") ~ "#003f5c",
      str_detect(term, "beat_score") ~ "#ff6361",
      TRUE ~ "grey"
    )
  ) %>%
    mutate(
    term = case_when(
      term == "(Intercept)" ~ "(Intercept)",
      term == "experiment_group" ~ "L1",
      term == "condition" ~ "Stress",
      term == "dprime_battery_up_dur" ~ "Duration d'",
      term == "duration" ~ "Duration",
      term == "dprime_battery_up_formants" ~ "Formants d'",
      term == "stress_score" ~ "Word Stress",
      term == "dprime_battery_up_ptich" ~ "Pitch d'",
      term == "melody_score" ~ "Melody",
      term == "beat_score" ~ "Rhythm",
      term == "mean_working_memory" ~ "Working Mem",
      term == "drift_rate" ~ "Drift Rate",
      term == "lextale_score" ~ "LexTALE",
      term == "experiment_group:condition" ~ "L1:Stress",
      term == "condition:experiment_group" ~ "L1:Stress",
      term == "dprime_battery_up_dur:duration" ~ "Duration d':Duration",
      term == "dprime_battery_up_formants:stress_score" ~ "Formants d':Word Stress",
      term == "dprime_battery_up_ptich:pitch_range" ~ "Pitch d':Pitch Range",
      term == "dprime_battery_up_risetime:amplitude_raw" ~ "Rise Time d':Amplitude",
      term == "melody_score:pitch_range" ~ "Melody:Pitch Range",
      term == "duration:beat_score" ~ "Duration:Rhythm",
      term == "mean_working_memory:drift_rate" ~ "Working Mem:Drift Rate",
      term == "mean_working_memory:lextale_score" ~ "Working Mem:LexTALE",
      term == "drift_rate:lextale_score" ~ "Drift Rate:LexTALE",
      term == "mean_working_memory:drift_rate:lextale_score" ~ "Working Mem:Drift Rate:LexTALE",
      term == "pitch_range" ~ "Pitch Range",
      term == "dprime_battery_up_risetime" ~ "Rise Time d'",
      term == "pitch_range:melody_score" ~ "Pitch Range:Melody",
      term == "amplitude_raw" ~ "Amplitude",
      term == "pitch_range:dprime_battery_up_ptich" ~ "Pitch Range:Pitch d'",
      term == "beat_score:duration" ~ "Rhythm:Duration",
      TRUE ~ term
    )
)%>%
  mutate(
...
id_gam_mod_out<-ggplot(id_gam, aes(x = term, y = estimate, color = color)) +
  geom_errorbar(aes(ymin = estimate - 1.96 *std.error, ymax = estimate + 1.96 *std.error), width = 0.4,color="black") +
  ...
  facet_grid(Looks ~ Phrase,scales="free_x")

id_gam_mod_out
ggsave("viz/id_gam_mod_out.png", id_gam_mod_out, width = 12, height = 6, dpi = 300)

write.csv(id_gam, file = "../data/id_gam_models.csv", row.names = FALSE)
```

#individual differences (possibel cleanup required below. not deleted to ensure system is checked before deletion)
...
run_lasso_gam <- function(data, response, time_var, random_effect, alpha = 1) {
  ...
  fixed_effects <- setdiff(names(data), c(response, time_var, random_effect))
  
  interaction_terms <- combn(fixed_effects, 2, function(x) paste(x, collapse = "*"))
  lasso_data <- model.matrix(as.formula(paste(response, "~", 
...
```{r}
run_lasso_gam(target_id_0, response = "value", time_var = "time_cats", random_effect = "participant.private.id")
```
